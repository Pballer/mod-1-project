...
def read_movie_data():
    '''Load movie data'''
    imdb_title_basics = pd.read_csv('./data_exploration/data/imdb.title.basics.csv.gz')
    imdb_title_ratings = pd.read_csv('./data_exploration/data/imdb.title.ratings.csv.gz')
    tn_movie_budgets = pd.read_csv('./data_exploration/data/tn.movie_budgets.csv.gz')
...
def merge_basics_with_ratings(imdb_title_basics, imdb_title_ratings):
    ...
    imdb_basics_rating = pd.merge(imdb_title_basics, imdb_title_ratings, on=['tconst'], how='outer', indicator=True)
...
def format_currency_fields(tn_movie_budgets):
    ...
    tn_movie_budgets.loc[:, 'production_budget':'worldwide_gross'] = \
        tn_movie_budgets.loc[:, 'production_budget':'worldwide_gross'].replace('[^.0-9]', '', regex=True).astype(int)

def calculate_roi(tn_movie_budgets):
    '''Calculate ROI for domestic and world'''
    tn_movie_budgets['dom_roi'] = round(((tn_movie_budgets.domestic_gross - tn_movie_budgets.production_budget)
                                         / tn_movie_budgets.production_budget) * 100, 2)
    tn_movie_budgets['world_roi'] = round(((tn_movie_budgets.worldwide_gross - tn_movie_budgets.production_budget)
                                           / tn_movie_budgets.production_budget) * 100, 2)
...
def remove_invalid_revenues(imdb_tn_merged):
    ...
    imdb_tn_current = imdb_tn_merged.loc[imdb_tn_merged.start_year < 2020].copy()
    imdb_tn_current = imdb_tn_current.loc[(imdb_tn_current.domestic_gross != 0)
...
def melt_genres(imdb_tn_cleaned):
    '''Split genres column into multiple rows.  Similar to melt function in pandas'''
    genres_movie = pd.concat([pd.Series(row['tconst'], row['genres'].split(','))
...
def drop_unused_columns(imdb_tn_by_genre):
    imdb_tn_by_genre.drop(columns=['_merge', 'movie', 'start_year'], inplace=True)
...
def transform(imdb_title_basics, imdb_title_ratings, tn_movie_budgets):
    ...
    imdb_basics_rating = merge_basics_with_ratings(imdb_title_basics, imdb_title_ratings)
    ...
    imdb_tn_primary_title = merge_imdb_with_tn(imdb_basics_rating, tn_movie_budgets, 'primary_title')

    # Merge imdb with tn on 'original_title' column
    imdb_tn_original_title = merge_imdb_with_tn(imdb_basics_rating, tn_movie_budgets, 'original_title')

    #Concat primary title with original title merged data.
    imdb_tn_merged = concat_imdb_tn_data(imdb_tn_primary_title, imdb_tn_original_title)
